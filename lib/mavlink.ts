...
import { uint8_t, uint16_t, x25crc, MSG_ID_MAGIC_NUMBER, MavLinkPacketRegistry } from 'mavlink-mappings'
...
/**
 * Magic number mapping. Defaults to `MSG_ID_MAGIC_NUMBER`.
 */
let mavlinkMagicMap = new Map<number, number>();
loadDefaultRegistry();

/**
 * Load the default mavlink registry. Done by default.
 */
export function loadDefaultRegistry() {
  mavlinkMagicMap.clear()

  Object.entries(MSG_ID_MAGIC_NUMBER)
      .forEach(([msgId, magic]) => {
        mavlinkMagicMap.set(Number(msgId), magic)
      })
}

/**
 * Load a custom mavlink registry. This will overwrite the current one.
 * @param registry custom Mavlink packet registry
 */
export function loadCustomRegistry(registry: MavLinkPacketRegistry) {
  mavlinkMagicMap.clear()

  for (const key in registry) {
    const data = registry[key]
    const msgId = Number(key)
    mavlinkMagicMap.set(msgId, data.MAGIC_NUMBER)
  }
}
...
export class MavLinkPacket {
  ...
  debug() {
    ...
    // @ts-ignore
    + `magic: ${mavlinkMagicMap.get(this.header.msgid)} (${hex(mavlinkMagicMap.get(this.header.msgid))}), `
      + `crc: ${hex(this.crc, 4)}`
    // @ts-ignore
...
export class MavLinkPacketSplitter extends Transform {
  ...
  private validatePacket(buffer: Buffer, Protocol: MavLinkProtocolConstructor) {
    ...
    // @ts-ignore
    const magic = mavlinkMagicMap.get(header.msgid)
    if (magic) {
