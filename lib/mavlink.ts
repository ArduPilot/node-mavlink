...
import { uint8_t, uint16_t, x25crc, MSG_ID_MAGIC_NUMBER } from 'mavlink-mappings'
...
/**
 * Custom registry type, generated by the pymavlink library.
 */
export type CustomRegistry = Map<number, new (system_id: number, component_id: number) => MavLinkData>;

/**
 * Load a custom mavlink registry. This will overwrite the current one.
 * @param registry custom Mavlink packet registry
 */
export function loadCustomRegistry(registry: CustomRegistry) {
  mavlinkMagicMap.clear()

  Array.from(registry.entries())
      .forEach(([msgId, msgBuilder]) => {
        mavlinkMagicMap.set(msgId, msgBuilder.prototype as typeof MavLinkData.MAGIC_NUMBER);
      })
}
